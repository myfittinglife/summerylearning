package com.example.administrator.summarylearning.recyclerview.customlayout;
...
/**
*  @Author      LD
*  @Time        2018.11.19  11:19
*  @Describe    自定义布局实现滑动删除（摘自同宗汇）后续补充，理解代码内容并修改，当出现滑动的标签时点击其他item,该item收回
*  @Modify
*/
public class MySwipeItemLayout extends FrameLayout {
    ...
    public MySwipeItemLayout(Context context) {
        this(context, null);
    }

    public MySwipeItemLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public MySwipeItemLayout(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_UP:
                if (mIsDragged) {
                    mDragHelper.processTouchEvent(ev);      //*处理相应的TouchEvent方法
                    mIsDragged = false;
                }
                break;
            default:
    ...
    public interface SwipeListener {
        void onSwipeOpen(MySwipeItemLayout view);

        void onSwipeClose(MySwipeItemLayout view);
    }
}